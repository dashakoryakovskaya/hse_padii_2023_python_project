        ...
        if call.data == "predict":
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text="Описание моделей:\n\n"
                                       "🤌*catboost*\nпредсказывается каждый день, "
                                       "подходит для предсказаний на длительный период\n\n"
                                       "🤌*lama*\nпредсказывается каждый день, "
                                       "высокая точность предсказания больших трат\n\n"
                                       "🤌*arima*\nпредсказывается каждый четвертый день"
                                       ", высокая точность предсказания характера трат\n\n"
                                       "🤌*prophet-1*\nпредсказывается каждый четвертый день, "
                                       "подходит для предсказания тренда трат или среднего значения\n\n"
                                       "🤌*prophet-2*\nпредсказывается каждый день, "
                                       "подходит для предсказаний на небольшой период\n"
                                       "\nВыберите модель:",
